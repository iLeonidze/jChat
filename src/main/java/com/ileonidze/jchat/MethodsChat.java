...
class MethodsChat {
    static String createChat(String sessionHash, String opponentUserID, String chatName) {
        ...
        if (opponentUserObject == null || opponentUserObject.getID() == null || ownerUserObject == null || ownerUserObject.getID() == null)
            return null;
            ...
            if (thisChat.getMembersIDs().size() > 0) {
                for (String userID : thisChat.getMembersIDs()) {
                    ...
                    if (user.getChatsIDs().contains(chatID))
                        VDB.users.set(VDB.users.indexOf(user), user.removeFromChatsIDs(chatID));
                }
            }
    ...
    static VDBChat getChat(String sessionHash, String chatID) {
        ...
        if (chatObject == null || chatObject.getMembersIDs() == null || chatObject.getOwnerID() == null)
            return new VDBChat();
        if (sessionUserObject.getID().equals(chatObject.getOwnerID()) || sessionUserObject.getAccessLevel() > 0)
            return chatObject;
    ...
    static boolean modifyChatMember(String sessionHash, String chatID, String opponentUserID, String action) {
        ...
        VDBUser sessionUserObject = MethodsUser.getSessionUser(sessionHash);
        VDBUser opponentUserObject = MethodsUser.getUserByID(sessionHash, opponentUserID);
        if (opponentUserObject == null || opponentUserObject.getID() == null || sessionUserObject == null || sessionUserObject.getID() == null)
            return false;
        ...
        switch (action) {
            case "invite":
                ...
                break;
            case "kick":
                if (!chatObject.getOwnerID().equals(sessionUserObject.getID()) && sessionUserObject.getAccessLevel() < 1)
                    return false;
                if (chatObject.getMembersIDs().contains(opponentUserID))
                    VDB.chats.set(chatIterator, chatObject.removeFromMembersIDs(opponentUserID));
                if (opponentUserObject.getChatsIDs().contains(chatID))
                    VDB.users.set(VDB.users.indexOf(opponentUserObject), opponentUserObject.removeFromChatsIDs(chatID));
                break;
            default:
                return false;
        }
    ...
    static String getRenameError(String thisSession, String chatID, String chatName) {
        ...
        if (userWhoRename == null || userWhoRename.getID() == null)
            return "Current session is invalid. Try to relogin.";
        if (!renamingChat.getOwnerID().equals(userWhoRename.getID()) && userWhoRename.getAccessLevel() < 1)
