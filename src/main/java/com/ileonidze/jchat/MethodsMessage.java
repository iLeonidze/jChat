...
class MethodsMessage {
    static String getSendError(String thisSession,String chatID,String body){
        ...
        if(!chatToSend.getMembersIDs().contains(userToSendFrom.getID())&&!chatToSend.getOwnerID().equals(userToSendFrom.getID())&&userToSendFrom.getAccessLevel()<1) return "Access denied";
        int chatIterator = -1;
    ...
    static VDBMessage getMessageInfo(String thisSession,String chatID,String messageID){
        ...
        if(!chatToLoadFrom.getMembersIDs().contains(userWhoLoad.getID())&&!chatToLoadFrom.getOwnerID().equals(userWhoLoad.getID())&&userWhoLoad.getAccessLevel()<1) return null;
        int chatIterator = -1;
        for(int i=0;i<VDB.chats.size();i++){
            VDBChat chat = VDB.chats.get(i);
            if(chat.getID().equals(chatID)){
        ...
        if(!messageToDelete.getOwnerID().equals(userWhoLoad.getID())&&!chatToLoadFrom.getOwnerID().equals(userWhoLoad.getID())&&userWhoLoad.getAccessLevel()<1) return null;
    ...
    static String getDeleteError(String thisSession,String chatID,String messageID){
        ...
        if(userWhoDelete == null || userWhoDelete.getID() == null) return "Current session is invalid. Try to relogin.";
        if(!chatToDeleteFrom.getMembersIDs().contains(userWhoDelete.getID())&&!chatToDeleteFrom.getOwnerID().equals(userWhoDelete.getID())&&userWhoDelete.getAccessLevel()<1) return "Access denied";
        int chatIterator = -1;
        for(int i=0;i<VDB.chats.size();i++){
            VDBChat chat = VDB.chats.get(i);
            if(chat.getID().equals(chatID)){
        ...
        if(!messageToDelete.getOwnerID().equals(userWhoDelete.getID())&&!chatToDeleteFrom.getOwnerID().equals(userWhoDelete.getID())&&userWhoDelete.getAccessLevel()<1) return "Access denied";
    ...
    static String getModifyError(String thisSession,String chatID,String messageID,String body){
        ...
        if(userWhoModify == null || userWhoModify.getID() == null) return "Current session is invalid. Try to relogin.";
        if(!chatToModifyFrom.getMembersIDs().contains(userWhoModify.getID())&&!chatToModifyFrom.getOwnerID().equals(userWhoModify.getID())&&userWhoModify.getAccessLevel()<1) return "Access denied";
        int chatIterator = -1;
        for(int i=0;i<VDB.chats.size();i++){
            VDBChat chat = VDB.chats.get(i);
            if(chat.getID().equals(chatID)){
        ...
        if(!messageToModify.getOwnerID().equals(userWhoModify.getID())&&!chatToModifyFrom.getOwnerID().equals(userWhoModify.getID())&&userWhoModify.getAccessLevel()<1) return "Access denied";
    ...
    static String getForwardError(String thisSession,String chatForwardFromID,String messageID,String chatForwardToID){
        ...
        if(userWhoForward == null || userWhoForward.getID() == null) return "Current session is invalid. Try to relogin.";
        if(!chatForwardingFrom.getMembersIDs().contains(userWhoForward.getID())&&!chatForwardingFrom.getOwnerID().equals(userWhoForward.getID())&&userWhoForward.getAccessLevel()<1) return "Access to chat #"+chatForwardFromID+" denied";
        ...
        if(chatForwardFromID.equals(chatForwardToID)){
            ...
            if(!chatForwardTo.getMembersIDs().contains(userWhoForward.getID())&&!chatForwardTo.getOwnerID().equals(userWhoForward.getID())&&userWhoForward.getAccessLevel()<1) return "Access to chat #"+chatForwardToID+" denied";
        }
